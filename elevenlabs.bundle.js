var ElevenLabs = (() => {
  ...
  __export(index_exports, {
    ...
    DEFAULT_INPUT_CHUNK_DURATION_MS: () => DEFAULT_INPUT_CHUNK_DURATION_MS,
    RealtimeConnection: () => RealtimeConnection,
    RealtimeEvents: () => RealtimeEvents,
    Scribe: () => ScribeRealtime,
    SessionConnectionError: () => SessionConnectionError,
    TextConversation: () => TextConversation,
    VoiceConversation: () => VoiceConversation,
    WebRTCConnection: () => WebRTCConnection,
    WebSocketConnection: () => WebSocketConnection,
    createConnection: () => createConnection,
    postOverallFeedback: () => postOverallFeedback
  });
...
`
  ...
  // node_modules/@elevenlabs/client/dist/utils/uploadFile.js
  ...
  async function uploadFile({ conversationId, origin: rawOrigin, file, filename: filenameOverride }) {
    const origin = (rawOrigin ?? HTTPS_API_ORIGIN2).replace(/^wss:\/\//, "https://").replace(/^ws:\/\//, "http://");
    ...
    const response = await fetch(`${origin}/v1/convai/conversations/${conversationId}/files`, { method: "POST", body });
    if (!response.ok) {
      const message = await extractApiErrorMessage(response);
    ...
    async handleClientToolCall(event) {
      if (Object.prototype.hasOwnProperty.call(this.options.clientTools, event.client_tool_call.tool_name)) {
        try {
          ...
          this.onError(`Client tool execution failed with following error: ${e2?.message}`, {
            clientToolName: event.client_tool_call.tool_name
          });
          this.connection.sendMessage({
            type: "client_tool_result",
            tool_call_id: event.client_tool_call.tool_call_id,
            result: `Client tool execution failed: ${e2?.message}`,
            is_error: true
          });
        }
      } else {
  ...
  // node_modules/@elevenlabs/client/dist/runtime.js
  ...
  var TextConversation = class _TextConversation extends BaseConversation {
    ...
    static async startSession(options) {
      ...
      try {
        ...
        conversation = new _TextConversation(fullOptions, connection);
  ...
  var VoiceConversation = class _VoiceConversation extends BaseConversation {
    ...
    static async startSession(options) {
      ...
      try {
        if (!setupStrategy) {
          throw new Error('No voice session setup strategy registered. Import the platform-specific entry point (e.g. @elevenlabs/client via the "browser" export).');
        }
        sessionSetup = await setupStrategy(fullOptions);
        conversation = new _VoiceConversation(fullOptions, sessionSetup.connection, sessionSetup.input, sessionSetup.output, sessionSetup.playbackEventTarget, sessionSetup.detach);
        fullOptions.onConversationCreated?.(conversation);
        conversation.markConnected();
    ...
    getInputByteFrequencyData() {
      this.inputFrequencyData ??= new Uint8Array(_VoiceConversation.FREQUENCY_BIN_COUNT);
      this.input.getByteFrequencyData(this.inputFrequencyData);
      return this.inputFrequencyData;
    }
    getOutputByteFrequencyData() {
      this.outputFrequencyData ??= new Uint8Array(_VoiceConversation.FREQUENCY_BIN_COUNT);
  ...
  function normalizeError(error) {
    if (typeof error === "object" && error !== null && "message_type" in error && "error" in error && typeof error.message_type === "string" && typeof error.error === "string") {
      return error;
    }
    return {
      message_type: "error",
